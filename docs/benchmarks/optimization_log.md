...
| `runtime-hud` | `4ace070`, `a381fad`, `4278db8` | unified viewer | Normalize both modes to one transparent HUD; show mode, camera profile, model, screen FPS, and model latency; replace the vendor watermark with raw pixels | overlapping mode-specific metric overlays and black panel | shared transparent fields in both modes; Mode 1 and Mode 2 screenshots verified | n/a | n/a | none | metrics/composition only; masks unchanged outside the watermark patch | accept |
| `reliable-desktop-restart` | `fddfed7`, `e6653c8`, `dbd6251`, `2ccc1ca`, `997fb4b` | Thor launch/container | Add idempotent restart, inherited/fallback GNOME discovery, Docker/camera waits, RealSense discovery, camera-frame readiness, and 10-second progress | manual environment/export plus silent cold-load and stale-launch failures | actual reboot and forced cold recreation both opened mode 2/TV5M at 848x480@60; forced cold reported ready after the 70-second update | n/a | n/a | cold GI load still takes roughly 70–90 s | same engines, precision, and camera profile | accept |
