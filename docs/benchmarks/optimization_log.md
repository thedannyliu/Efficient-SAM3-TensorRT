...
| `two-fps-hud` | `4ace070`, `a381fad` | unified viewer | Normalize both modes to two persistent pale-yellow metrics and cover the vendor FPS watermark | overlapping mode-specific metric overlays | `Screen FPS` plus `Model-only FPS` only; transient menu/status text retained | n/a | n/a | none | metrics only; masks unchanged | accept |
